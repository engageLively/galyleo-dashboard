...
export class PublishedDashboard extends Morph {
  ...
  async loadDashboardFromUrl (url) {
    this.url = url;
  ...
  // Make sure a descriptor is OK.  This makes sure it has Tables, Views, Filters,
  // and Charts
  // Returns {valid: true/false, message: <explanatory message if false}.
  // Primarily designed for loadDashboardFromUrl
  ...
  _checkValid_ (dashboardObject) {
    ...
    const expectedFields = ['tables', 'views', 'filters', 'charts', 'fill'];
    const optionalFields = ['morphs', 'numMorphs'];
    const allFields = expectedFields + optionalFields;
    ...
    if (missingFields.length == 0 && newFields.length == 0) {
      ...
      return { valid: false, message: `File was missing fields ${missingFields} and had unexpected fields ${newFields}` };
    }
  }

  // get fields from an object.  This is a utility used by _prepareSerialization_
  // to pull out morphic and text properties
  // parameters:
  //   fields: fields to pull from the object
  ...
  // The fields to pull from a morph
  get _morphicFields_ () {
    return ['rotation', 'scale', 'clipMode', 'opacity'];
  }

  // The fields to pull from a Text
  get _textFields_ () {
    return ['fontFamily', 'fontSize', 'fontStyle',
      'fontWeight', 'textAlign', 'lineWrapping', 'fixedHeight',
      'fixedWidth', 'textString', 'textDecoration'];
  }
  ...
  // parameters:
  //    rgba: A string serializing the rgba object or an object with fields r,g,b,a
  ...
  _setComplexFields_ (aMorph, fieldDescriptor) {
    aMorph.position = Point.fromLiteral(fieldDescriptor.position);
    aMorph.extent = Point.fromLiteral(fieldDescriptor.extent);
    aMorph.fill = this._color_(fieldDescriptor.fill);
    const newBorder = {
      borderRadius: fieldDescriptor.border.borderRadius,
      width: fieldDescriptor.border.width,
      style: fieldDescriptor.border.style,
      color: {}
    };

    const sides = ['top', 'bottom', 'left', 'right'];
    sides.forEach(side => newBorder.color[side] = this._color_(fieldDescriptor.border.color[side]));
    ...
    if (fieldDescriptor.hasOwnProperty('dropShadow')) {
      ...
      aMorph.dropShadow.color = this._color_(fieldDescriptor.dropShadow.color);
    }
  }
  ...
  _setComplexTextFields_ (textMorph, descriptor) {
    textMorph.fontColor = this._color_(descriptor.fontColor);
  ...
  _setFields_ (morph, descriptor, fields) {
    fields.forEach(field => morph[field] = descriptor[field]);
  }
  ...
  async _restoreFromSaved_ (storedForm /* Now as an object, not a JSON string */) {
    ...
    try {
      ...
      if (storedForm.fill) {
        this.fill = this._color_(storedForm.fill);
      }
